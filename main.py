...
if option == "device":
    ...
    extrap_device_revOp(ns, config.nextrap, config.nz, config.nt, config.nw, \
        config.nx, config.M, w_op_fs_forw, pulse_forw_fs, w_op_fs_back, \
        pulse_back_fs, image)
elif option == "device_sh_revOp":
    print("Extrapolation on device using shared-memory (rearranged operators)")
    extrap_device_sh_revOp(ns, config.nextrap, config.nz, config.nt, config.nw, \
        config.nx, config.M, w_op_fs_forw, pulse_forw_fs, w_op_fs_back, \
        pulse_back_fs, image)
elif option == "host":
